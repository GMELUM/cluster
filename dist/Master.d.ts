...
export declare type BridgeMaster = {};
declare type TCallbackEvents<C extends BridgeCluster, T extends Record<string, Array<Record<string, any>>> = C> = <K extends keyof T, V extends T[K]>(cluster: Worker, type: K, value: V[0], reply?: (value: V[1]) => void) => void;
declare type TEvents<T extends BridgeCluster> = (callback: TCallbackEvents<T>) => void;
...
declare class Master<M extends BridgeMaster, C extends BridgeCluster, MT extends Record<string, Array<Record<string, any>>> = M> {
    ...
    newCluster: (opt?: Record<string, string>) => Worker;
